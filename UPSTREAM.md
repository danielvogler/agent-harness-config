...
| | Upstream ECC's `deep-research` | Ours, now `custom-deep-research` |
| --- | --- | --- |
...
| Module | `research-apis` (not in our profile) | `custom-skills` |
...
**Resolution:** our skill lives at `custom/skills/custom-deep-research/` and the command
points at it. The `custom-` prefix is exactly the reserved namespace
...
The command has no leading `ultrawork:` line: that is an
[oh-my-openagent](https://github.com/code-yeongyu/oh-my-openagent) keyword, meaningless in all
five target harnesses.
